...
first_ratified: 01/11/2017 # (please don't alter this field)
...
- In particular, the Co-op will retain the following in Tenant files:
    ...
    - Record of Welfare files held (files kept separately; reference or case numbers should be placed in Tenant files for easy referral)
...
1. Either directly, or coordinating with a member of the Tech group if required, the secretarial group should ensure that the relevant policy web-page is accordingly amended.
2. The documents should use html markup that is consistent with other documents on the site
3. The office staff should be notified when this has been done so they can update the Policies folder with a pdf print from the webpage.

In the general case, policies exist to give clear guidance on the operation of a Working Group, its responsibilities, and any expectations with respect to reporting to the General Meeting.

Community agreements are any piece of guidance or any enforceable decision that the General Meeting takes that falls outside of a Working Group e.g. An approach to noise issues in the community or an agreement about the use of shared spaces or resources. Any agreements shall be kept on file alongside Working Group policies.

Whenever a policy is reviewed, a new policy drafted, or amendments made to an existing policy, it is crucial that the General Meeting in which the policy is presented has a clear outline of what has been changed, what the intention of the new draft is, and who has been involved in its creation, so that an informed decision can be made on accepting the policy.
...
- Each General Meeting will consider all minutes received by the membership since the last GM and take a vote on
approving them, along with any amendments agreed upon.
